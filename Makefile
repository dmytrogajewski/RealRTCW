...
LLAMA_LIBS = -L$(CURDIR)/code/llama.cpp/build/bin -lllama -lggml -lggml-base -lggml-cpu -lpthread -lstdc++

ifdef MINGW
$(B)/$(BASEGAME)/qagame_sp_$(SHLIBNAME): $(Q3GOBJ)
	$(echo_cmd) "LD $@"
	$(Q)$(CXX) $(CFLAGS) $(SHLIBLDFLAGS) -o $@ $(Q3GOBJ) $(LIBS) $(LLAMA_LIBS)
else
...
$(B)/$(BASEGAME)/game/steamshim_child.o: $(STEAMSHIMDIR)/steamshim_child.c
	$(DO_GAME_CC)
