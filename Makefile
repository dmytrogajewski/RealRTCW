...
Q3UIOBJ_ = \
  $(B)/$(BASEGAME)/ui/ui_graphics_enhancements.o \
...
install-steam:
	...
	if [ -d "$(CURDIR)/code/renderer/shaders" ]; then \
		...
		mkdir -p /tmp/rtcw_shaders_pk3/shaders; \
		cp $(CURDIR)/code/renderer/shaders/*.vert $(CURDIR)/code/renderer/shaders/*.frag /tmp/rtcw_shaders_pk3/shaders/ 2>/dev/null || true; \
		cd /tmp/rtcw_shaders_pk3 && zip -r "$$STEAMDIR/main/zzz_glsl_shaders.pk3" shaders/; \
		rm -rf /tmp/rtcw_shaders_pk3; \
		echo "GLSL shaders installed."; \
	else \
		echo "Warning: GLSL shaders not found, skipping..."; \
	fi; \
	echo "Creating enhanced rendering config..."; \
	echo "// Enhanced Rendering Settings - Generated by make install-steam" > "$$STEAMDIR/main/realrtcw_enhanced.cfg"; \
	echo "seta r_glsl \"1\"" >> "$$STEAMDIR/main/realrtcw_enhanced.cfg"; \
	echo "seta r_hdr \"1\"" >> "$$STEAMDIR/main/realrtcw_enhanced.cfg"; \
	echo "seta r_hdrExposure \"0.0\"" >> "$$STEAMDIR/main/realrtcw_enhanced.cfg"; \
	echo "seta r_hdrGamma \"2.2\"" >> "$$STEAMDIR/main/realrtcw_enhanced.cfg"; \
	echo "seta r_tonemap \"1\"" >> "$$STEAMDIR/main/realrtcw_enhanced.cfg"; \
	echo "seta r_ssao \"1\"" >> "$$STEAMDIR/main/realrtcw_enhanced.cfg"; \
	echo "seta r_ssaoRadius \"0.5\"" >> "$$STEAMDIR/main/realrtcw_enhanced.cfg"; \
	echo "seta r_ssaoIntensity \"2.0\"" >> "$$STEAMDIR/main/realrtcw_enhanced.cfg"; \
	echo "seta r_pbr \"1\"" >> "$$STEAMDIR/main/realrtcw_enhanced.cfg"; \
